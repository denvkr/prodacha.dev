...
*/
				...
				//$url='http://phpshop.dev/shop/CID_21.html';
...
foreach( $tools as $tool ){
	...
	{				
		...
		foreach ( $products as $product ) {
			...
			$product_id=substr($product,0,$product_id_end_pos);
		...
		//составл€ем массив со всемы значени€ми из выбранной характеристики даже если их несколько		
		foreach($id0_2 as $id0_2_item) {
				...
				do {
					...
					if (!$item_existing) {
						$names_2[$cnt][$row1_2['id']]=$row1_2['name'];
	...
	foreach ($filterconfs as $filterconf) {
		//echo '--'.$filterconf['id'].' $cnt='.$har_arr_cnt.'--<br />';	
	}
	//print_r($filterconfs);
	if ( $filterconf['id']==1 || ($filterconf['id']==2 && ($har_arr_cnt==1 || $har_arr_cnt==0)) ){
