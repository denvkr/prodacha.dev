...
function UpdateDelivery(xid) {
                ...
                if ( ((delivery_options!==null && delivery_options.selected===true) || delivery_options===null) && legal_form[0].checked===true && (Number(xid)===10 || Number(xid)===13) ) {
                    ...
                    if (($("td input[name='order_metod']:eq(0)").length && 
                        ...
                        $("form[name='forma_order']>table:eq(0) tr:eq(9) td:eq(1)").html("<input type=\"radio\" name=\"order_metod\" checked value=\"3\" onClick=\"document.getElementById('bin').style.display='block';document.getElementById('bic').style.display='none';\"><label>Наличная оплата</label><br>"+
                            ...
                            "<div id=\"order_metod_div\" style=\"display:table-cell;\"><input type=\"radio\" name=\"order_metod\" value=\"26\" onClick=\"document.getElementById('bin').style.display='block';document.getElementById('bic').style.display='none';\"><label>Оплата в магазине картой VISA, Mastercard</label><br>");//+ "<input type=\"radio\" name=\"order_metod\" value=\"15\" onClick=\"document.getElementById('bin').style.display='block';document.getElementById('bic').style.display='none';\"><label>Онлайн платеж.</label><br></div>"
    ...
    add_popover();
    if ($.trim($('.city_link').eq(0).text())==="Чебоксары"){
        $('div[name="seldelivery"]:eq(1)>a:first')[0].click();
    }
    if ($.trim($('.city_link').eq(0).text())==="Санкт-Петербург"){
    }    
}
