...
class PHPShopShop extends PHPShopShopCore {
    ...
    function CID() {
		...
		*/
        ...
        if ( intval($this->category)==400 ) {
        	$this->PHPShopOrm->sql = 'select 400 as id,'
				  ."name,"
				  ."num,"
				  .'327 as parent_to,'
				  ."yml,"
    ...
    function CID_Category() {
		...
		if (intval($this->PHPShopNav->getId())==400) {
		...
		$cid_id==337				
	        ...
	        if ($cid_id==32) {
	        	if ($this->PHPShopNav->getPage()=='') {
	        		$sql="select distinct id,32 as category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array from ".$GLOBALS['SysValue']['base']['products']." where dop_cat like "
	        		."'%#32#%' and enabled='1' and parent_enabled='0' order by ".$orderby." LIMIT 1,".$this->num_row;
	        	} else if ($this->PHPShopNav->isPageAll()) {
	        		$sql="select distinct id,32 as category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array from ".$GLOBALS['SysValue']['base']['products']." where dop_cat like "
	        		."'%#32#%' and enabled='1' and parent_enabled='0' order by ".$orderby;
	        	} else {
	        		$num_row=$this->PHPShopNav->getPage()*$this->num_row;
	        		//echo $num_row.' '.$this->num_row;
	        		$sql_2=" LIMIT ".($num_row-$this->num_row).",".$this->num_row;
	        		$sql="select distinct id,32 as category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array from ".$GLOBALS['SysValue']['base']['products']." where dop_cat like "
	        		."'%#32#%' and enabled='1' and parent_enabled='0' order by ".$orderby.$sql_2;
	        	}
	        } else	        
	        if ($cid_id==154) {
	        	if ($this->PHPShopNav->getPage()=='') {
	        		$sql="select distinct id,154 as category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array from ".$GLOBALS['SysValue']['base']['products']." where dop_cat like "
	        		."'%#154#%' and enabled='1' and parent_enabled='0' order by ".$orderby." LIMIT 1,".$this->num_row;
	        	} else if ($this->PHPShopNav->isPageAll()) {
	        		$sql="select distinct id,154 as category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array from ".$GLOBALS['SysValue']['base']['products']." where dop_cat like "
	        		."'%#154#%' and enabled='1' and parent_enabled='0' order by ".$orderby;
	        	} else {
	        		$num_row=$this->PHPShopNav->getPage()*$this->num_row;
	        		//echo $num_row.' '.$this->num_row;
	        		$sql_2=" LIMIT ".($num_row-$this->num_row).",".$this->num_row;
	        		$sql="select distinct id,154 as category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array from ".$GLOBALS['SysValue']['base']['products']." where dop_cat like "
	        		."'%#154#%' and enabled='1' and parent_enabled='0' order by ".$orderby.$sql_2;
	        	}
	        } else if ($cid_id==400) {
				if ($this->PHPShopNav->getPage()=='') {
					$sql="select distinct id,400 as category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array from ".$GLOBALS['SysValue']['base']['products']." where category in "
					."(400,120,38,62) and enabled='1' and parent_enabled='0' order by ".$orderby." LIMIT 1,".$this->num_row;
				} else if ($this->PHPShopNav->isPageAll()) {
					$sql="select distinct id,400 as category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array from ".$GLOBALS['SysValue']['base']['products']." where category in "
					."(400,120,38,62) and enabled='1' and parent_enabled='0' order by ".$orderby;				
				} else {
					$num_row=$this->PHPShopNav->getPage()*$this->num_row;
					//echo $num_row.' '.$this->num_row;
					$sql_2=" LIMIT ".($num_row-$this->num_row).",".$this->num_row;
					$sql="select distinct id,400 as category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array from ".$GLOBALS['SysValue']['base']['products']." where category in "
					."(400,120,38,62) and enabled='1' and parent_enabled='0' order by ".$orderby.$sql_2;
				}
			} else {
				if ($this->PHPShopNav->getPage()=='') {
			...
			if (($cid_id==30 || $cid_id==154 || $cid_id==32)  && $cell==1) {
				//echo 1;
