...
function delivery($obj,$deliveryID) {
    ...
    while($row = mysql_fetch_array($resultvariants)) {
		...
		//nah
		/*	if ($_SESSION['sincity']=="Санкт-Петербург") 
			{
				@$disp.='<option id="var_1" value="1">Доставка</option>
				<OPTION id="var_'.$row['id'].'111" value="'.$row['id'].'111" '.$chk.'>Самовывоз: Лахтинский пр-т д.2 кор.1</OPTION>';
			}	
