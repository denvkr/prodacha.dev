...
class PHPShopShop extends PHPShopShopCore {
    ...
    function CID() {
		...
		*/
        ...
        if ( intval($this->category)==500 ) {
        	$this->PHPShopOrm->sql = 'select 500 as id,'
				  ."name,"
				  ."num,"
				  .'327 as parent_to,'
				  ."yml,"
				  ...
				  ."filtr,"
				  ."icon_description"
		         .' from ' . $this->getValue('base.categories') . ' where id in (120,62,38)';
    ...
    function CID_Category() {
        ...
        // Название категории
        $this->category_name = $this->PHPShopCategory->getName();
        //echo intval($this->PHPShopNav->getId());
		if (intval($this->PHPShopNav->getId())==500) {
			$where = " where skin_enabled!='1' and id in (120,62,38)";
		} else {
			// Условия выборки
		...
		if (intval($this->PHPShopNav->getId())==500) {
			$PHPShopOrm->sql='select id,'
				  ...
				  ."filtr,"
				  ."icon_description"
		         .' from ' . $this->getValue('base.categories') . $where;
        ...
        if (is_array($dataArray))
			...
			foreach ($dataArray as $row) {
				if ( 
					...
					$this->PHPShopNav->getId()==30 ||
					$this->PHPShopNav->getId()==32 ||
					$this->PHPShopNav->getId()==33 ||
					$this->PHPShopNav->getId()==34 ||
					$this->PHPShopNav->getId()==35 ||
					$this->PHPShopNav->getId()==36 ||
					$this->PHPShopNav->getId()==37 ||
					$this->PHPShopNav->getId()==38 ||
					$this->PHPShopNav->getId()==44 ||
					$this->PHPShopNav->getId()==60 ||
					$this->PHPShopNav->getId()==62 ||
					$this->PHPShopNav->getId()==77 ||
					$this->PHPShopNav->getId()==81 ||							
					$this->PHPShopNav->getId()==85 ||
					$this->PHPShopNav->getId()==88 ||
					$this->PHPShopNav->getId()==96 ||
					$this->PHPShopNav->getId()==97 ||
					$this->PHPShopNav->getId()==98 ||
					$this->PHPShopNav->getId()==99 ||
					$this->PHPShopNav->getId()==120	||
					$this->PHPShopNav->getId()==122	||
					$this->PHPShopNav->getId()==123 ||							
					$this->PHPShopNav->getId()==134 ||
					$this->PHPShopNav->getId()==141 ||
					$this->PHPShopNav->getId()==142 ||
					$this->PHPShopNav->getId()==143 ||
					$this->PHPShopNav->getId()==154 ||
					$this->PHPShopNav->getId()==172	||
					$this->PHPShopNav->getId()==186 ||
					$this->PHPShopNav->getId()==190 ||
					$this->PHPShopNav->getId()==191 ||
					$this->PHPShopNav->getId()==211 ||
					$this->PHPShopNav->getId()==215 ||
					$this->PHPShopNav->getId()==224	||
					$this->PHPShopNav->getId()==227 ||
					$this->PHPShopNav->getId()==228 ||
					$this->PHPShopNav->getId()==234 ||							
					$this->PHPShopNav->getId()==247 ||							
					$this->PHPShopNav->getId()==248 ||
					$this->PHPShopNav->getId()==249 ||
					$this->PHPShopNav->getId()==254 ||
					$this->PHPShopNav->getId()==252 ||
					$this->PHPShopNav->getId()==256 ||
					$this->PHPShopNav->getId()==257 ||
					$this->PHPShopNav->getId()==258 ||
					$this->PHPShopNav->getId()==270 ||
					...
					$this->PHPShopNav->getId()==332	||
					$this->PHPShopNav->getId()==333	||
					$this->PHPShopNav->getId()==334	||														
					$this->PHPShopNav->getId()==335 ||
					$this->PHPShopNav->getId()==336 ||
					$this->PHPShopNav->getId()==337	||
					$this->PHPShopNav->getId()==350 ||
					$this->PHPShopNav->getId()==351 ||
					$this->PHPShopNav->getId()==352 ||
					$this->PHPShopNav->getId()==353 ||
					$this->PHPShopNav->getId()==354 ||
					$this->PHPShopNav->getId()==355	||
					$this->PHPShopNav->getId()==356 ||
					$this->PHPShopNav->getId()==382	||
					$this->PHPShopNav->getId()==414	||
					$this->PHPShopNav->getId()==415	||
					$this->PHPShopNav->getId()==416	||
					$this->PHPShopNav->getId()==418	||
					$this->PHPShopNav->getId()==419	||
					$this->PHPShopNav->getId()==420	||
					$this->PHPShopNav->getId()==421	||
					$this->PHPShopNav->getId()==422	||
					$this->PHPShopNav->getId()==423	||
					$this->PHPShopNav->getId()==424	||
					$this->PHPShopNav->getId()==425	||
					$this->PHPShopNav->getId()==500
					)) {
		...
		if 	(	$cid_id==5 || 
	        ...
	        if (!empty($_GET['s']) and is_numeric($_GET['s']) and !empty($_GET['f']) and is_numeric($_GET['f'])) {
	         ...
	         if ($cid_id==32 || 
				...
				if ($this->PHPShopNav->getPage()=='') {
					$sql="select distinct id,500 as category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array from ".$GLOBALS['SysValue']['base']['products']." where category in "
					."(500,120,38,62) and enabled='1' and parent_enabled='0' order by ".$orderby." LIMIT 1,".$this->num_row;
				} else if ($this->PHPShopNav->isPageAll()) {
					$sql="select distinct id,500 as category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array from ".$GLOBALS['SysValue']['base']['products']." where category in "
					."(500,120,38,62) and enabled='1' and parent_enabled='0' order by ".$orderby;				
				} else {
					$num_row=$this->PHPShopNav->getPage()*$this->num_row;
					//echo $num_row.' '.$this->num_row;
					$sql_2=" LIMIT ".($num_row-$this->num_row).",".$this->num_row;
					$sql="select distinct id,500 as category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array from ".$GLOBALS['SysValue']['base']['products']." where category in "
					."(500,120,38,62) and enabled='1' and parent_enabled='0' order by ".$orderby.$sql_2;
				}
			} else {
				if ($this->PHPShopNav->getPage()=='') {
					...
					$sql="select distinct id,category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array from ".$GLOBALS['SysValue']['base']['products']." where category in"
					...
					$sql="select distinct id,category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,outdated,analog,vendor,vendor_array from ".$GLOBALS['SysValue']['base']['products']." where category in"
					." (SELECT id FROM ".$GLOBALS['SysValue']['base']['categories']." where parent_to in"
					." (SELECT id FROM ".$GLOBALS['SysValue']['base']['categories']." where parent_to=".$cid_id
					." union all"
			...
			foreach ($res as $prod_row) {			
				...
				if ($cid_id==228) {
					...
					$productnotice='<input type="button" onclick="stihl_window(\''.$region_info.'\')"  value="'.$this->lang('stihl_string').'" />';
					...
					$productnotice='<input type="button" onclick="viking_window(\''.$region_info.'\')"  value="'.$this->lang('stihl_string').'" />';
					$comnotice='<div class="prev_price" style="font-size:11px !important;">'.$this->lang('sklad_mesage').'</div>';
					$outdated_style='';
				} else {
					// снят с производства
					if (!(empty($prod_row['sklad'])) && !(empty($prod_row['outdated']))) {
						$productnotice='<div class="prev_price" style="position:relative;display: inline-block;top:3px;margin:-15px 0px 0px 10px;font-size:12px !important;"><noindex>'.$this->lang('outdated_message').'</noindex></div>';
						$productnotice.='<div class="price_comlain" style="position:relative;display: inline-block;margin:0px 95px 0px 0px;font-size:13px !important;"><noindex>'.$this->lang('outdated_message2').'</noindex></div>';
						$productnotice.='<a id="analog_href'.$prod_row['id'].'" style="visibility: hidden;" href="http://test.prodacha.ru/shop/UID_'.$prod_row['analog'].'.html"></a>';
						$productnotice.='<script type="text/javascript">';
						$productnotice.='$(document).ready(function() {';
						$productnotice.='$(".addtochart > .price_comlain").click( function( event ) {';
				...
				if (($cid_id==30 ||
					...
					.'<span class="prev_price"><noindex>'.$comnotice_30.'</noindex></span> <span class="price">'.$mod_price.'<span class="smallfont">'.$this->PHPShopSystem->getDefaultValutaCode(true).'</span></span>'					
