...
class PHPShopDone extends PHPShopCore {
        ...
        if ($this->PHPShopCart->getNum() > 0) {
            if (PHPShopSecurity::true_param(@$_POST['mail'], @$_POST['name_person'], @$_POST['tel_name'])) { //, $_POST['adr_name']
                ...
                if (!empty($_SESSION['UsersId']) and PHPShopSecurity::true_num($_SESSION['UsersId'])) {
                	...
                	$search = array(' ', '-', '(', ')');
                	$this->set('UserTel',str_replace($search,"",@$_POST['tel_name']));
                }
    ...
    function mail() {
		...
		if ($_COOKIE['sincity']=="sp") {
			...
			$PHPShopMail= new PHPShopMail("prodacha21@gmail.com",$_POST['mail'],$title_adm,$content_adm);		
		} else {
			if (strpos($this->PHPShopDelivery->getCity(),'Минское')!==false) {
				$PHPShopMail= new PHPShopMail("minka@prodacha.ru",$_POST['mail'],$title_adm,$content_adm);
			} else
