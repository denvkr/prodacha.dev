...
class PHPShopDone extends PHPShopCore {
    ...
    function write() {
        ...
        if (preg_match('/^(Москва).*$/i',$this->PHPShopDelivery->getCity())===1){
            $pre_tk='Доставка по Москве и МО';
        }
        //$pre_tk=$this->PHPShopDelivery->getCity();
        // Данные покупателя
        $person = array(
	...
	//ob_end_clean();
        ...
        if (isset($_POST['firstname']) &&
           ...
           $this->status = array(
               ...
               "time" => "");
             */
            $person["adr_name"]="Транспортная компания:".$tk.
               " Имя:".@str_replace("_", "",$_POST['firstname']).
               " Отчество:".@str_replace("_", "",$_POST['middlename']).
               " Фамилия:".@str_replace("_", "",$_POST['lastname']).
               " Серия:".@str_replace("_", "",$_POST['pass_no1'].
           ...
           $this->status = array(
               ...
               "time" => "");
             */
           $person["adr_name"]="Транспортная компания: Почта России".
               " Имя:".@str_replace("_", "",$_POST['firstname']).
               " Отчество:".@str_replace("_", "",$_POST['middlename']).
               " Фамилия:".@str_replace("_", "",$_POST['lastname']).
               " Телефон грузополучателя:".$tel2.
