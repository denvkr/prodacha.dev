...
class PHPShopShop extends PHPShopShopCore {
    ...
    function UID() {
		...
		{	
			...
			$firstcreditpunch='
			<span class="addtochart buyincredit2" id="credit_'.$row['id'].'" style="left:-10px;">
			...
			foreach ($res as $prod_row) {			
				...
				if ($cid_id==228) {
					...
					if (!(empty($prod_row['sklad'])) && !(empty($prod_row['outdated']))) {
						...
						$productnotice.='<div id="price_comlain'.$prod_row['id'].'" class="price_comlain" style="position:relative;display: inline-block;margin:0px 95px 0px 0px;font-size:13px !important;"><noindex>'.$this->lang('outdated_message2').'</noindex></div>';
