...
class PHPShopShop extends PHPShopShopCore {
    ...
    function CID_Category() {
		...
		if (intval($this->PHPShopNav->getId())==473 || intval($this->PHPShopNav->getId())==474 || intval($this->PHPShopNav->getId())==475 ||
			...
			intval($this->PHPShopNav->getId())==509	) 
		{
		...
		if ($this->PHPShopNav->getId()==472 || $this->PHPShopNav->getId()==473 ||$this->PHPShopNav->getId()==474 ||
			...
			$this->PHPShopNav->getId()==496 || $this->PHPShopNav->getId()==497 || $this->PHPShopNav->getId()==498 ||
			$this->PHPShopNav->getId()==499 || $this->PHPShopNav->getId()==500 || $this->PHPShopNav->getId()==501 ||
			$this->PHPShopNav->getId()==502 || $this->PHPShopNav->getId()==509) {
	...
	function catalog_improvements() {
		...
		if 	(	$cid_id==5 || $cid_id==9 ||	$cid_id==16 || $cid_id==18 ||	
			...
			if ($cid_id==31 ||	$cid_id==32 || $cid_id==33 || $cid_id==34 || $cid_id==35 ||
				...
				$cid_id==483 || $cid_id==484 || $cid_id==485 || $cid_id==486 || $cid_id==487 || 
	         	...
	         	$cid_id==498 || $cid_id==499 || $cid_id==500 || $cid_id==501 || $cid_id==502
	        		...
	        		*/
	        		$sql="select distinct id,category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array,sortorder from (select distinct id,".$cid_id." as category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array,1 as sortorder from ".$GLOBALS['SysValue']['base']['products']." where dop_cat like "
	        		."'%#".$cid_id."#%' and parent_enabled='0' and sklad='0' and enabled='1' and outdated='0'".
	        		" union all ".
	        		"select distinct id,".$cid_id." as category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array,2 as sortorder from ".$GLOBALS['SysValue']['base']['products']." where dop_cat like "
	        		."'%#".$cid_id."#%' and parent_enabled='0' and sklad='1' and enabled='1' and outdated='0'".
	        		" union all ".
	        		"select distinct id,".$cid_id." as category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array,3 as sortorder from ".$GLOBALS['SysValue']['base']['products']." where dop_cat like "
				...
				if ($this->PHPShopNav->getPage()=='') {
					...
					$sql="select distinct id,category,name,content,price,price_n,sklad,p_enabled,enabled,uid,num,pic_small,parent_enabled,parent,price2,price3,price4,price5,dop_cat,outdated,analog,vendor,vendor_array, case when sklad='0' and outdated='0' then 1 when sklad='1' and outdated='0' then 2 when sklad='1' and outdated='1' then 3 end as sortorder from ".$GLOBALS['SysValue']['base']['products']." where category in"
			...
			foreach ($res as $prod_row) {			
				...
				if ( $cid_id==228 || $cid_id==459 || $cid_id==461 || $cid_id==464 ) {
				...
				//формируем карточку для viking
					...
					if (!(empty($prod_row['sklad'])) && !(empty($prod_row['outdated']))) {
						...
						if ( !(empty($prod_row['analog'])) ) {
							...
							//вывод пустышки если нет ссылки на анлог
				...
				if (($cid_id==30 || $cid_id==31 || $cid_id==32 || $cid_id==33 ||
					...
					.'<span class="prev_price"><noindex>'.$comnotice.$comnotice_30.'</noindex></span> <span class="price">'.$mod_price.'<span class="smallfont">&nbsp'.$this->PHPShopSystem->getDefaultValutaCode(true).'</span></span>'					
					...
					if ($cell==1 || $cell==2 || $cell==3 || $cell==4) {
						...
						if ($cnt == 1) {
							...
							.$productnotice
							.'</span>'.$ComEndNotice
							.'<span class="price" '.$outdated_style.'>'.$mod_price.'<span class="smallfont">&nbsp'.$this->PHPShopSystem->getDefaultValutaCode(true).'</span></span>'
							.'<div style="clear:both"></div>'
