...
function userorderpaymentlink_mod_liqpay_hook($obj, $PHPShopOrderFunction) {
    ...
    if ($PHPShopOrderFunction->getParam('statusi') == $option['status']) {
      ...
      <result_url>http://' . $_SERVER['SERVER_NAME'] . '/phpshop/modules/liqpay/payment/result.php</result_url>
      <server_url>' . urlencode('http://' . $_SERVER['SERVER_NAME'] . '/success/?payment=liqpay&inv_id=' . $inv_id) . '</server_url>
