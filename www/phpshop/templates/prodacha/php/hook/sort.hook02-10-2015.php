...
4. 24 месяца со дня продажи на всю аккумуляторную технику, электрические и бензиновые снегоуборщики.</pre><br />';  	
    ...
    //$tab6_html='test';
    $obj->set('warrantyInfo',stripslashes($tab6_html));
...
function display_custom_catalogList_hook($obj,$row,$rout) {
      if($rout == 'END') {
          $obj->set('display_custom_catalogList_hook_test','test',true);
          //return true;
      }  
}
*/
      ...
      if($rout == 'END') {
        ...
        foreach ($ceo_custom_catalog_productname as $ceo_custom_catalog_productname_item) {
            if (in_array($obj->PHPShopNav->getId(),$ceo_custom_catalog_productname_item)) {
                $productname_mod=str_replace(strtolower($ceo_custom_catalog_productname_item['str1']),strtolower($ceo_custom_catalog_productname_item['str2']),$obj->get('productName'));
...
function ceo_price_hook($obj,$row,$rout){
    ...
    if (strpos($_SERVER['REQUEST_URI'],"UID")===false) {
        if ($obj->PHPShopCategory->getParam('num_row')==1)
            return '<span style="font:18pt Arial Regular;color:#588910;">ЦЕНА:</span>'.$row;
        else {
            return '<span style="display:block;font:11pt Arial Regular;color:#6c4b46;">ЦЕНА:</span>'.$row;    
        }        
    } else
        return $row;
}
