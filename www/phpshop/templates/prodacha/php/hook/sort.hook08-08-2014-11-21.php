...
function display_tovar_delivery_hook($obj, $row) {
	...
	if ($price>=1000){
		$tab7_html.=PHPShopText::br(1).'</br>'.$GLOBALS['SysValue']['lang']['warranty_tab_string2'].PHPShopText::nbsp(1);
		//выбираем город
	...
	switch ($_COOKIE['sincity']) {
		case 'm': $city='Москва,Московская область';
			...
			if ($price>=1000 && $price<=4999) {
				...
				$dostavka5=$GLOBALS['SysValue']['lang']['delivery_tab_string5'].PHPShopText::br(1).'</br>'.PHPShopText::br(1).'</br>'.$GLOBALS['SysValue']['lang']['delivery_tab_string3'];
			}
			...
			if ($price>=10000 && $price<=19999) {
				$sub_sql1=" where (enabled='1' and id in ('11','3','23','25')) order by city asc";
				$dostavka5=$GLOBALS['SysValue']['lang']['delivery_tab_string5'].PHPShopText::br(1).'</br>'.PHPShopText::br(1).'</br>'.$GLOBALS['SysValue']['lang']['delivery_tab_string3'];
			}
			if ($price>=20000 && $price<=49999) {
				$sub_sql1=" where (enabled='1' and id in ('11','3','26','27','28','38','39','40')) order by city asc";
				$dostavka5=$GLOBALS['SysValue']['lang']['delivery_tab_string5'].PHPShopText::br(1).'</br>'.PHPShopText::br(1).'</br>'.$GLOBALS['SysValue']['lang']['delivery_tab_string3'];
			}
			if ($price>=50000) {
				$sub_sql1=" where (enabled='1' and id in ('11','3','29','30','31','32','33','34','35','36','37','42')) order by city asc";
			}
			if ($price<1000) {
				//return true;
		...
		case 'sp':  $city='Санкт-Петербург,Ленинградская область';
			...
			if ($price>=10000 && $price<=19999) {
				$sub_sql1=" where (enabled='1' and id in ('11','44','46','48')) order by city asc";
				$dostavka5=$GLOBALS['SysValue']['lang']['delivery_tab_string5'].PHPShopText::br(1).'</br>'.PHPShopText::br(1).'</br>'.$GLOBALS['SysValue']['lang']['delivery_tab_string3'];
			}
			if ($price>=20000 && $price<=49999) {
				$sub_sql1=" where (enabled='1' and id in ('11','44','49','50','51','61','62')) order by city asc";
				$dostavka5=$GLOBALS['SysValue']['lang']['delivery_tab_string5'].PHPShopText::br(1).'</br>'.PHPShopText::br(1).'</br>'.$GLOBALS['SysValue']['lang']['delivery_tab_string3'];
			}
			if ($price>=50000) {
				$sub_sql1=" where (enabled='1' and id in ('11','44','52','53','54','55','56','57','58','59','60','63')) order by city asc";
			}		
			if ($price<1000) {
				//return true;
				$sub_sql1=" where enabled='-1'";
	...
	while ($row_tab7=mysql_fetch_assoc($res_tab7)) {
    	...
    	switch ($row_tab7['city']) {
    		...
    		case 'Москва за пределами МКАД от 41 до 50 км. от 50000 руб.';
    			$city='За пределами МКАД от 41 до 50 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД до 10 км. от 20000 руб.':
    			$city='За пределами КАД до 10 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД от 11 до 20 км. от 20000 руб.':
    			$city='За пределами КАД от 11 до 20 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД от 21 до 30 км. от 20000 руб.':
    			$city='За пределами КАД от 21 до 30 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД от 31 до 40 км. от 20000 руб.':
    			$city='За пределами КАД от 31 до 40 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД от 41 до 50 км. от 20000 руб.':
    			$city='За пределами КАД от 41 до 50 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД до 10 км. от 5000 до 9999 руб.':
    			$city='За пределами КАД до 10 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД до 10 км. от 10000 руб.';
    			$city='За пределами КАД до 10 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД от 11 до 20 км. от 5000 до 9999 руб.';
    			$city='За пределами КАД от 11 до 20 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД от 11 до 20 км. от 10000 руб.';
    			$city='За пределами КАД от 11 до 20 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД от 51 до 60 км. от 50000 руб.';
    			$city='За пределами КАД от 51 до 60 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД от 61 до 70 км. от 50000 руб.';
    			$city='За пределами КАД от 61 до 70 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД от 71 до 80 км. от 50000 руб.';
    			$city='За пределами КАД от 71 до 80 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД от 81 до 90 км. от 50000 руб.';
    			$city='За пределами КАД от 81 до 90 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД от 91 до 100 км. от 50000 руб.';
    			$city='За пределами КАД от 91 до 100 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД до 10 км. от 50000 руб.';
    			$city='За пределами КАД до 10 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД от 21 до 30 км. от 50000 руб.';
    			$city='За пределами КАД от 21 до 30 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД от 31 до 40 км. от 50000 руб.';
    			$city='За пределами КАД от 31 до 40 км.';
    			break;
    		case 'Санкт-Петербург за пределами КАД от 41 до 50 км. от 50000 руб.';
    			$city='За пределами КАД от 41 до 50 км.';
    			break;    			
    	}
    ...
    $tab7_html.=PHPShopText::br(1).'</br>'.$dostavka5;
...
function display_tovar_samovyvoz_hook($obj, $row) {
	...
	$tab7_1_html.=PHPShopText::br(1).'</br>'.$GLOBALS['SysValue']['lang']['samovyvoz_tab_string2'].$tovar.$GLOBALS['SysValue']['lang']['samovyvoz_tab_string3'];
	...
	$tab7_1_html.='<thead><tr><th style="text-align:left">Адрес:</th></tr></thead><tbody>';
	
	while ($row_tab7=mysql_fetch_assoc($res_tab7)) {
		$tab7_1_html.='<tr>';
		$tab7_1_html.='<td>'.str_replace('Самовывоз','',$row_tab7['city']).'</td>';
		$tab7_1_html.='</tr>';
	}
