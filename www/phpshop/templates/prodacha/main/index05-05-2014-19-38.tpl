...
<head>
	...
	<script language="javascript">
		...
		'java/ap.js',
		'http://yes-credit.su/crem/js/jquery-ui-1.8.23.custom.min.js',
		'http://yes-credit.su/crem/js/crem.js'
		];
	...
	<script src="@php echo $GLOBALS['SysValue']['dir']['templates'].chr(47).$_SESSION['skin'].chr(47); php@javascript/script.js?v1" type="text/javascript"></script>
	...
	@php
		if ($_COOKIE['sincity']=="sp")
			echo "<style>.store, .shop, .shop_cheb{display:none;}</style>";
		else
			echo "<style>.shop_spb{display:none;}</style>";
			
		if ($_COOKIE['sincity']=="chb")
			echo "<style>.store, .shop, shop_spb{display:none;}</style>";
		else
			echo "<style>.shop_cheb{display:none;}</style>";			
	php@
...
if(!isset($city)){
	...
	<!--slider end-->
				@php			
				if ($_COOKIE['sincity']=="m" || $_COOKIE['sincity']=="chb" || !isset($_COOKIE['sincity'])) {
