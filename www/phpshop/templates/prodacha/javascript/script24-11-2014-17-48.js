	...
	*/
	...
	$('.left .menu .mainmenu > li').click(function ( event ) {
		...
		*/
			...
			if ($($(this).next('li')).children('span').text().toLowerCase()=="мотоблоки") {			
				if ($(this).next('li').css("display")=="block") {
					...
					$(this).next('li').css("display","block");
						//$(this).next('li').css("border-left","1px solid red");
						//$(this).next('li').css("border-right","1px solid red");						
				}
			}
		...
		/*
		if ($(this).children("span").text().toLowerCase()=='снегоуборочная техника') {
		...
		*/
		...
		/*
		if ($(this).children("span").text().toLowerCase()=='генераторы') {
		...
		*/
		...
		if ($(this).children("span").text().toLowerCase()=='техника для газона') {
			...
			if ($($($(this).nextAll('li').children('span')).get(9)).text().toLowerCase()=="высоторезы") {			
			...
			if ($($($(this).nextAll('li').children('span')).get(10)).text().toLowerCase()=="сенокосилки фронтальные") {			
			...
			if ($($($(this).nextAll('li').children('span')).get(11)).text().toLowerCase()=="подрезчики края газона") {			
			...
			if ($($($(this).nextAll('li').children('span')).get(12)).text().toLowerCase()=="садовые катки и валики") {			
		...
		/*
		if ($(this).children("span").text().toLowerCase()=='цепные пилы') {
		...
		*/
		...
		if ($(this).children("span").text().toLowerCase()=='благоустройство участка') {
			...
			if ($($($(this).nextAll('li').children('span')).get(14)).text().toLowerCase()=="воздуходувы") {			
			...
			if ($($($(this).nextAll('li').children('span')).get(15)).text().toLowerCase()=="мотобуры, бензобуры") {			
			...
			if ($($($(this).nextAll('li').children('span')).get(16)).text().toLowerCase()=="виброплиты") {			
			...
			if ($($($(this).nextAll('li').children('span')).get(17)).text().toLowerCase()=="дровоколы, измельчители") {			
			...
			spanelements.each(function( index ) {
				...
				if ($(this).find('span:first').text().toLowerCase()=='воздуходувы') {
				...
				if ($(this).find('span:first').text().toLowerCase()=='мотобуры, бензобуры') {
				...
				if ($(this).find('span:first').text().toLowerCase()=='дровоколы, измельчители') {
		...
		/*
		if ($(this).children("span").text().toLowerCase()=='минимойки') {
		...
		*/
		/*
		if ($(this).children("span").text().toLowerCase()=='садовые тракторы') {
		...
		*/
		...
		/*
		if ($(this).children("span").text().toLowerCase()=='техника karcher') {
		...
		*/
		...
		/*
		if ($(this).children("span").text().toLowerCase()=='техника viking') {
					document.getElementById('fade').style.display='block';
					//$('.mainmenu').find(".submenuhead").fadeOut('fast');
					//$(this).children(".submenuhead").fadeIn('fast');
				if ( target.is("span")) {
		...
		*/
		//console.log($(this).children("span").text().toLowerCase().substring(0,13));
	});	
	...
	$.ajax({
		...
		success: function(json) {
			if (json['ok'] == '1'){
				...
				html +=	'<input type="tel" '+yo_phone_top+' id="yo_phone" name="yo_phone" onblur="checkresult_value(this);" value="" placeholder="+79101234567" pattern="^[\+0-9]{7,12}$" maxlength="11"/>';
				html +=	'</div>';	
				html +=	'<input id="cv3" type="hidden" name="cv3" value="nohspamcode" />';
				html +=	'<div class="in_fast_order_window">';
				html +=	'<input type="button" '+popup_ok_top+' class="popup_ok" id="fast_order_ok" value="OK" onclick="check_fast_order_values();"/>'; //onclick="check_fast_order_values()"			
...
function check_yo_name_value() {
	//console.log($('#yo_name').length);
}

function check_fast_order_values() {
...
function checkresult_value(field) {
	...
	if ( field.id=='yo_name' ) {
		...
		if (regExpr.test(field.value)==false) {
			...
			$(document).ready(function(){
					//setTimeout('$("#yo_name").focus();',1);
					//setTimeout('$("#yo_name").select();',1);
			});
		} else {
			$(document).ready(function(){
				//$('#yo_phone').attr('disabled', false);
				//setTimeout("$('#yo_phone').focus();",1);
				//setTimeout("$('#yo_phone').select();",1);
			});
		}
	}
	if ( field.id=='yo_phone' ) {
		...
		if (regExpr.test(field.value)==false) {
			...
			$(document).ready(function(){
				//$('#yo_name').attr('disabled', false);
				//setTimeout("$('#fast_order_ok').focus();",1);
				//setTimeout("$('#fast_order_ok').select();",1);
			});
		}
	}	
}
...
function dirPath(){
	//ROOT_PATH
    return '/'+window.location.host+'/www';
}
