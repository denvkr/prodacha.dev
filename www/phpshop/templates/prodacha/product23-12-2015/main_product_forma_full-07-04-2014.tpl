...
<div class="productbox">
						...
						<td>
							<div class="addchart_block_table_price">
									<span class="price margino">
										...
										<div>@productPrice@ <span class="smallfont">&nbsp@productValutaName@</span></div>
									</span>
	...
	$sql="select id,name,sklad,outdated from ".$SysValue['base']['products']." where category in (select id from ".$SysValue['base']['categories']." where name like '%stihl%' or name like '%viking%') and id=".$us_;
    ...
    if ($stihl_catalog_search==true || $viking_catalog_search==true) {
		...
		if ($row_sklad==true && $row_outdated==false) {
			...
			if ($viking_catalog_search==true) {
				echo '<span class="addtochart notice" style="'.$sdvig_knopki_vlevo.'"> <input type="button" style="font-size:9px;" onclick="viking_window(\\''.$region_info.'\\')"  value="'.$SysValue['lang']['stihl_string'].'" /> </span>';		
			} else {
				echo '<span class="addtochart notice" style="'.$sdvig_knopki_vlevo.'"> <input type="button" style="font-size:9px;" onclick="stihl_window(\\''.$region_info.'\\')"  value="'.$SysValue['lang']['stihl_string'].'" /> </span>';
			}
