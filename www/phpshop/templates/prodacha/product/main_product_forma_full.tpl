...
<div class="productbox">
			...
			<div class="addchart_block">
				...
				<table class="addchart_block_table">
					...
					<tr>
						...
						<td>
							<span class="quantity">Кол-во: <input class="quantity" id="n@productUid@" type="num" maxlength="5" size="3" value="1" name="n@productUid@" /></span>
						</td>
    ...
    if ($stihl_catalog_search==true || $viking_catalog_search==true) {
		...
		if ($row_sklad==true && $row_outdated==false) {
			...
			echo '<a id="analog_href'.$GLOBALS['SysValue']['other']['productId'].'" href="//prodacha.ru/shop/UID_'.$analog.'.html">АНАЛОГ</a>';
			...
			echo '<a id="analog_href'.$GLOBALS['SysValue']['other']['productId'].'" href="//prodacha.ru/shop/UID_'.$analog.'.html" onclick="return false;">АНАЛОГ</a>';
			echo '<span class="outdated_message" style="position:relative;font-size:11px !important;left: 11px;"><!--noindex-->'.$GLOBALS['SysValue']['lang']['outdated_message3'].'<!--/noindex--></span>';
                        echo '</span>';
		} else {
			if ($viking_catalog_search==true) {
		...
		if ($row_outdated==false) {
			...
			echo '	return false;';
			echo '});';
			echo '$("#complainlayer-container .modal-close").click( function( event ) {';
			echo '	$("#complainlayer-container, #light_box1").hide();';
			echo '	return false;';
			echo '});});';
			echo '</script>';
		}
		...
		$row_sklad=false;
		$price_n=false;
		$sql="select sklad,price_n,outdated,analog from ".$SysValue['base']['products']." where id=".$GLOBALS['SysValue']['other']['productId'];
		$res=mysql_query($sql);
		while ($catalog_id_rows=mysql_fetch_row($res)) {
		...
		if (($row_sklad==true && $price_n==true && $outdated==false) || ($row_sklad==true && $price_n==false && $outdated==false)) {
			...
			echo '@ComEndNotice@';
			echo '<span class="buybuttons" style="'.$sdvig_knopki_vlevo_old.'">';
			echo '<a id="analog_href'.$GLOBALS['SysValue']['other']['productId'].'" href="//'.$GLOBALS['SysValue']['other']['serverName'].'/shop/UID_'.$analog.'.html">АНАЛОГ</a>';
			echo '<span class="outdated_message" style="position:relative;font-size:11px !important;left: 11px;"><!--noindex-->'.$GLOBALS['SysValue']['lang']['outdated_message3'].'<!--/noindex--></span>';
                        echo '</span>';
                        /*
			echo '<script type="text/javascript">';
			...
			echo '@ComEndNotice@';
			echo '<span class="buybuttons inactive" style="'.$sdvig_knopki_vlevo_old.'">';
			echo '<a id="analog_href'.$GLOBALS['SysValue']['other']['productId'].'" href="//'.$GLOBALS['SysValue']['other']['serverName'].'/shop/UID_'.$analog.'.html" onclick="return false;">АНАЛОГ</a>';
			echo '<span class="outdated_message" style="position:relative;font-size:11px !important;left: 11px;"><!--noindex-->'.$GLOBALS['SysValue']['lang']['outdated_message3'].'<!--/noindex--></span>';
                        echo '</span>';
                        /*
			echo '<script type="text/javascript">';
	...
	<div tabindex="-1" class="simplemodal-wrap" style="height: 100%; outline: 0px; width: 100%; overflow: visible;">
		...
		$(document).ready(function() {
			if (window.location.hash=="#tab7") {
				$(".tabNavigation li:eq(2) a:eq(0)").click();
				$(".tabNavigation li:eq(2) a:eq(0)").focus();
			}
