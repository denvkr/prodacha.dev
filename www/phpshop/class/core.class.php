...
class PHPShopCore {
    ...
    function getNavigationPath($id) {
		...
		//echo 'need2hide='.$id.in_array($id,$need2hide);
        if  ( !empty($id) ) // AND (!in_array($id,$need2hide))
		 {
            $PHPShopOrm->comment="Навигация";
            //$v=$PHPShopOrm->select(array('name,id,parent_to'),array('id'=>'='.$id),false,array('limit'=>1));
            $where=' where id='.$id;
            $PHPShopOrm->sql="select id,name,parent_to from ". $this->getValue('base.categories') . $where;
        ...
        if (!empty($sql)) {
        	...
        	if ( (!isset($_GET['s'])) AND (!isset($_GET['f'])) AND (!isset($_GET['v'])) )
        	{
        		if (strpos($sql,"order by num desc")!==false){
        			$sql = str_replace(" order by num desc", " order by sorting asc,sortorder asc,price asc,num desc ",$sql);
        		}
        		if (strpos($sql,"order by name")!==false) {
        			$sql = str_replace(" order by name", " order by sorting asc,sortorder asc,price asc,name ",$sql);
        		}
        	}
        	//echo $sql;        	 
            $this->PHPShopOrm->sql = "select *, case when sklad='0' and outdated='0' then 1 when sklad='1' and outdated='0' then 2 when sklad='1' and outdated='1' then 3 end as sortorder from " . $this->objBase . " where " . $sql . " limit " . $option['limit'];
        }
