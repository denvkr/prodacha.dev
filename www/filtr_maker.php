...
foreach( $tools as $tool ){
	...
	if (is_array($filterconfs['2'])) {
		if ( $filterconfs['2']['id']==1 || ($filterconfs['2']['id']==2 && ($har_mainarr_cnt==0 || ($har_mainarr_cnt==1 && $har_arr_cnt==1))) ){
			$option_part='<div class="fmenu"><a href="javascript: void(0);" onclick="changeV('.$step.')" class="visible_all" id="visible_all'.$step.'">–азвернуть/—крыть фильтр: ' . $tool['name'] . '</a></div>';
