<?php
	...
	*/
	...
	if ($_REQUEST['only_read']==1) {
		...
		if ($q1_yes=='true') {
				echo $myvoting_class->get_voted_session_status($q1_session,$session_end_time);
				if ($myvoting_class->get_voted_session_status($q1_session,$session_end_time)==0){
					$my_vot_arr=$myvoting_class->add_question_data($q1_page,1,0);
					$myvoting_class->alter_voted_session_status($q1_session,$session_end_time);
				}
		}
		if ($q1_no=='true') {
				echo $myvoting_class->get_voted_session_status($q1_session,$session_end_time);
